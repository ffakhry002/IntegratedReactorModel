...
def run_depletion(output_dir=None):
    ...
    # Create log file
    log_path = os.path.join(output_dir, "depletion_log.txt")
    with open(log_path, 'w') as f:
        # Log initial information
        f.write("Pin Cell Depletion Calculation Log\n")
        f.write("=================================\n\n")

        # Log heavy metal mass
        hm_mass = operator.heavy_metal
        f.write(f"Heavy Metal Mass: {hm_mass:.4f} grams\n")

        # Log fuel pin volume
        fuel_volume = inputs['fuel_height'] * 100 * np.pi * (inputs['r_fuel'] * 100)**2  # in cm³
        f.write(f"Fuel Pin Volume: {fuel_volume:.4f} cm³\n")

        # Log pin power
        power_density = inputs['power_density']  # W/gHM
        total_power = power_density * hm_mass  # W
        f.write(f"Pin Power: {total_power:.4f} W\n\n")

        f.write("Depletion Results:\n")
        f.write("------------------\n")
    ...
    try:
        ...
        # Load results and append k-eff values to log
        ...
        power_density = inputs['power_density']  # W/gHM
        total_power = power_density * hm_mass  # W
        burnup = time_days * total_power/1e6 / (hm_mass/1000)  # MWd/kgHM

        with open(log_path, 'a') as f:
            for t, b, k in zip(time_days, burnup, keff):
