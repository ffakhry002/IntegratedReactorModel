...
def three_stage_optimization(X_train, y_train, model_class, model_type='xgboost', n_jobs=-1):
    ...
    FIXED: Proper parameter handling and early stopping
    """
    ...
    best_params_so_far = {}
    best_score_so_far = float('-inf')  # Using -inf since we minimize negative MSE
    ...
    # Helper function to extract parameter value regardless of prefix
    def get_param_value(params, param_name):
        """Get parameter value handling both prefixed and non-prefixed versions"""
        if f'estimator__{param_name}' in params:
            return params[f'estimator__{param_name}']
        elif param_name in params:
            return params[param_name]
        else:
            raise KeyError(f"Cannot find {param_name} in parameters: {list(params.keys())}")

    # Helper function to check if parameters are prefixed
    def has_estimator_prefix(params):
        """Check if parameters have estimator__ prefix"""
        return any(key.startswith('estimator__') for key in params.keys())
    ...
    def safe_fit_with_progress(search_cv, X, y, stage_name):
        ...
        try:
            ...
            if hasattr(search_cv, 'n_iter'):
                total_fits = search_cv.n_iter * search_cv.cv
            else:
                # Grid search
                total_combinations = 1
                for param_values in search_cv.param_grid.values():
                    total_combinations *= len(param_values)
                total_fits = total_combinations * search_cv.cv
    ...
    # Add estimator__ prefix for multi-output models if needed
    if is_multi_output and model_type in ['xgboost', 'svm', 'neural_net']:
        param_distributions = {f'estimator__{k}': v for k, v in base_param_distributions.items()}
    else:
        param_distributions = base_param_distributions

    print(f"\n Random Search Parameters:")
    print(f"   - Candidates to test: 150")
    print(f"   - Cross-validation folds: 10")
    print(f"   - Parameter prefix: {'estimator__' if is_multi_output and model_type in ['xgboost', 'svm', 'neural_net'] else 'none'}")
    ...
    if success and best_random_params:
        best_params_so_far = best_random_params.copy()
        best_score_so_far = best_random_score
        print(f"\n Stage 1 Results:")
        print(f"   Best score: {best_random_score:.6f}")
        print(f"   Best params sample: {list(best_random_params.items())[:3]}...")
    else:
        print("\n❌ Stage 1 failed. Returning default parameters.")
        return {}, None

    stage1_time = time.time() - stage1_start
    print(f"   Stage 1 took {stage1_time/60:.1f} minutes")
    ...
    has_prefix = has_estimator_prefix(best_random_params)
    print(f"   Parameter format: {'prefixed' if has_prefix else 'non-prefixed'}")
    ...
    try:
        if model_type == 'xgboost':
            ...
            n_est = get_param_value(best_random_params, 'n_estimators')
            depth = get_param_value(best_random_params, 'max_depth') if 'max_depth' in str(best_random_params) else 20
        ...
        if has_prefix:
            param_grid = {f'estimator__{k}': v for k, v in base_grid.items()}
        else:
        ...
        if success and best_grid_params:
            ...
            if best_grid_score > best_score_so_far:
                best_params_so_far = best_grid_params.copy()
                best_score_so_far = best_grid_score
                print(f"\n✅ Grid search improved performance!")
            else:
                print(f"\n⚠️  Grid search did not improve performance.")
                print(f"   Random: {best_random_score:.6f} vs Grid: {best_grid_score:.6f}")
                print(f"   Keeping random search parameters.")
                # Early stopping - skip Stage 3
                stage2_time = time.time() - stage2_start
                print(f"   Stage 2 took {stage2_time/60:.1f} minutes")
                return best_params_so_far, None
        ...
        print("   Skipping to final results with Stage 1 parameters.")
        return best_params_so_far, None
    ...
    try:
        ...
        if model_type == 'xgboost':
            n_est = get_param_value(best_params_so_far, 'n_estimators')
            depth = get_param_value(best_params_so_far, 'max_depth')
            lr = get_param_value(best_params_so_far, 'learning_rate')
            subsample = get_param_value(best_params_so_far, 'subsample')
            colsample = get_param_value(best_params_so_far, 'colsample_bytree')
            ...
            n_est = get_param_value(best_params_so_far, 'n_estimators')
            min_split = get_param_value(best_params_so_far, 'min_samples_split')
            min_leaf = get_param_value(best_params_so_far, 'min_samples_leaf')
            ...
            if 'max_depth' in str(best_params_so_far):
                depth = get_param_value(best_params_so_far, 'max_depth')
                if depth:
                    base_spaces['max_depth'] = Integer(max(5, depth-3), min(50, depth+3))

        elif model_type == 'svm':
            C = get_param_value(best_params_so_far, 'C')
            gamma = get_param_value(best_params_so_far, 'gamma')
            eps = get_param_value(best_params_so_far, 'epsilon')
            kernel = get_param_value(best_params_so_far, 'kernel')
            ...
            lr_init = get_param_value(best_params_so_far, 'learning_rate_init')
            alpha = get_param_value(best_params_so_far, 'alpha')
            layers = get_param_value(best_params_so_far, 'hidden_layer_sizes')
            activation = get_param_value(best_params_so_far, 'activation')

            base_spaces = {
                'learning_rate_init': Real(lr_init*0.5, lr_init*2.0, prior='log-uniform'),
        ...
        # Apply prefix if needed
        if has_prefix:
            search_spaces = {f'estimator__{k}': v for k, v in base_spaces.items()}
        else:
            search_spaces = base_spaces
        ...
        if success and best_bayes_params:
            if best_bayes_score > best_score_so_far:
                best_params_so_far = best_bayes_params.copy()
                best_score_so_far = best_bayes_score
                print(f"\n✅ Bayesian search improved performance!")
            else:
                print(f"\n⚠️  Bayesian search did not improve performance.")

    except Exception as e:
        print(f"\n❌ Error in Stage 3: {str(e)}")
        print("   Using best parameters from previous stages.")
    ...
    if best_random_score != 0:
        # Since we're minimizing negative MSE, improvement is when score gets more negative
        improvement = ((best_random_score - best_score_so_far) / abs(best_random_score)) * 100
    else:
        improvement = 0
    ...
    if improvement < 0:
        print("     Note: Negative improvement means performance degraded.")
        print("   Consider using Optuna optimization instead.")
    print(f" Total time: {total_time/60:.1f} minutes")

    # Clean up parameters for final model creation
    # Remove 'estimator__' prefix if present for the final parameters
    clean_params = {}
...
# ######## This is the three stage optimization function old just for checking sake
# import numpy as np
# from sklearn.model_selection import RandomizedSearchCV, GridSearchCV
# from skopt import BayesSearchCV
# from skopt.space import Real, Integer, Categorical
