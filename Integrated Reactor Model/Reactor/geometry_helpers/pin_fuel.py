...
def build_pin_cell_fuel_uni(mat_dict, is_enhanced=False):
    ...
    coolant_cell = openmc.Cell(fill=mat_dict[f"{inputs['coolant_type']} Coolant"], region=+r3)

    # Create universe
    pin_universe = openmc.Universe(name='fuel', cells=[fuel_cell, he_cell, zr_cell, coolant_cell])
    return pin_universe

def build_guide_tube_uni(mat_dict):
    ...
    r1 = openmc.ZCylinder(r=inputs['r_clad_inner'] * 100)  # m to cm
    r2 = openmc.ZCylinder(r=inputs['r_clad_outer'] * 100)  # m to cm
    ...
    # Create cells
    inner_coolant_cell = openmc.Cell(fill=mat_dict[f"{inputs['coolant_type']} Feed"], region=-r1)
    zr_cell = openmc.Cell(fill=mat_dict[clad_material], region=+r1 & -r2)
    outer_coolant_cell = openmc.Cell(fill=mat_dict[f"{inputs['coolant_type']} Coolant"], region=+r2)
...
def build_fuel_assembly_uni(mat_dict, position=None, is_enhanced=False):
    ...
    position : tuple, optional
        (i, j) position in core lattice. If provided, assigns unique IDs.
    ...
    guide_tube = build_guide_tube_uni(mat_dict)
    fuel = build_pin_cell_fuel_uni(mat_dict, is_enhanced)
    ...
    coolant_uni = openmc.Universe(cells=[openmc.Cell(fill=mat_dict[f"{inputs['coolant_type']} Coolant"])])
