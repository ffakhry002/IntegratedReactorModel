"""
Random sampling in lattice geometry space.
...
class RandomLatticeSampler(BaseSampler):
    """Random sampling directly in lattice configuration space for baseline comparison."""

    def __init__(self, use_6x6_restriction=False):
        super().__init__(use_6x6_restriction=use_6x6_restriction)
        self.method_name = "random_lattice"

    def sample(self, n_samples: int, n_runs: int = 10, base_seed: int = 42) -> Dict:
        """
        Perform random lattice sampling multiple times and return the most diverse set.
        ...
        """
        print(f"\nRunning Random Lattice {n_runs} times to find most diverse set...")
        print("Completely random selection of configurations")
        print("Provides baseline comparison for other methods")
    ...
    def _single_random_run(self, n_samples: int) -> List[int]:
        ...
        """
        n_configs = len(self.irradiation_sets)
