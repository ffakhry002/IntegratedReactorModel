"""
Sobol sequence sampling directly in 4D lattice configuration space.
...
class SobolLatticeSampler(BaseLatticeSampler):
    """Sobol sequence sampling in lattice space with symmetry awareness."""

    def __init__(self, use_6x6_restriction=False):
        super().__init__(use_6x6_restriction=use_6x6_restriction)
        self.method_name = "sobol_lattice"

    def sample(self, n_samples: int, n_runs: int = 10, base_seed: int = 42) -> Dict:
        """
        Perform Sobol lattice sampling with symmetry-aware matching.
        ...
        """
        print(f"\nRunning Sobol Lattice {n_runs} times...")
        print("Using symmetry-reduced configuration set with 8-way rotation matching")
        print("Using scrambled Sobol sequences for randomization")
        ...
        for run in range(n_runs):
            ...
            indices, avg_distance = self._single_sobol_lattice_run(n_samples, seed)
    ...
    def _single_sobol_lattice_run(self, n_samples: int, seed: int = None) -> Tuple[List[int], float]:
